      ...
      <div className="w-full">
        ...
        <div className="fade-up mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-white text-xl font-semibold tracking-tight">List</h1>
              <p className="text-white/35 text-sm mt-0.5">{filtered.length} records · last updated just now</p>
            </div>
            <div className="flex items-center gap-2">
              <button
                disabled
                className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white/[0.05] border border-white/[0.08] text-white/50 hover:text-white/80 hover:bg-white/[0.08] text-sm font-medium transition-all"
              >
              ...
              <Link
                href={"/app/users/create?role=agent"}
        ...
        <div className="fade-up bg-[#0f1117] rounded-2xl border border-white/[0.07] overflow-hidden shadow-2xl">
              ...
              <tbody>
                {paginated.length === 0 ? (
                  ...
                  paginated.map((item, i) => {
                    const cat = categoryIcon["HR"] || { bg: "bg-white/[0.06]", icon: "•" };
                      ...
                      >
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-white/85 font-medium text-sm leading-tight">
                                {item?.firstName} {item?.lastName}
                              </p>
                              <p className="text-white/30 text-xs mt-0.5 mono">{item.userId}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3.5">
                        ...
                        <td className="px-4 py-3.5">
                          <p className="text-white/55 text-sm">{item.passport?.verified ? "Yes" : "No"}</p>
                        </td>
                        <td className="px-4 py-3.5">
                          <p className="text-white/55 text-sm">{item.pan?.verified ? "Yes" : "No"}</p>
                        </td>
                        {/* Date */}
                        <td className="px-4 py-3.5">
                          <p className="text-white/55 text-sm">{fmtTimeStampDate(item.createdAt)}</p>
                        </td>
          ...
          <div className="md:hidden divide-y divide-white/[0.04]">
            {paginated.length === 0 ? (
              <div className="text-center py-12 text-white/20 text-sm">No Records found</div>
            ) : (
              paginated.map((item) => {
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-white/30 text-xs mono">{item.userId}</span>
                      </div>
                    </div>
                  </div>
                );
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-white/[0.06]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-white font-semibold text-sm">{detailRow.userId} </p>
                </div>
              </div>
            ...
            <div className="px-6 py-6 border-b border-white/[0.06]">
              <div className="flex justify-between">
                <p className="text-white/35 text-xs uppercase tracking-wider mb-1">{detailRow?.role} </p>
            ...
            <div className="flex-1 px-6 py-4 space-y-5 overflow-y-auto">
              {/* info */}
              <h1 className="text-white text-sm mb-3">Basic Details</h1>
                ...
                <>
                  ...
                  <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] divide-y divide-white/[0.04] overflow-hidden">
                    {[["PAN", `${detailRow?.pan?.panNumber}`]].map(([k, v]) => (
                      <div key={k} className="flex items-center justify-between px-4 py-3">
                        <span className="text-white/35 text-sm capitalize">{k}</span>
                        <span className="text-white/75 text-sm font-medium mono">{!v ? "-" : v}</span>
                      </div>
                    ))}
